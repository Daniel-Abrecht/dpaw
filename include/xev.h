...
struct xev_event_extension {
  ...
  int extension_index;
  bool required;
  int(*init)(struct dpawin* dpawin, struct dpawin_xev*);
  int(*cleanup)(struct dpawin_xev*);
};
...
const struct xev_event_extension* dpawin_get_event_extension(const struct dpawin*, int extension);
